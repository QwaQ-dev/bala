...
export async function GET(request, { params }) {
  ...
  try {
    ...
    try {
      ...
      console.error("[Admin Course Get API] Ошибка парсинга JSON:", parseError.message);
      return new Response(
        JSON.stringify({
...
export async function PUT(request, { params }) {
  const { id } = params;
  console.log("[Admin Course Update API] Called for id:", id, "at", new Date().toISOString());
  try {
    const cookieHeader = request.headers.get("cookie") || "";
    ...
    const response = await fetch(`${BACKEND_URL}/api/v1/admin/course/update/${id}`, {
      method: "PUT",
      headers,
      credentials: "include",
      signal: controller.signal,
    ...
    try {
      ...
      console.error("[Admin Course Update API] Ошибка парсинга JSON:", parseError.message);
      return new Response(
        JSON.stringify({
          error: "Неверный формат ответа от бэкенда",
          details: responseText.slice(0, 100) + "...",
