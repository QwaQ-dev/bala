...
const CustomImage = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    const src = HTMLAttributes["src"] || `https://api.birlikbala.kz/uploads/articles/${path}`
    return [
      "img",
      {
        ...HTMLAttributes,
        ...
        ({ commands }) =>
          commands.insertContent({
            ...
            attrs: {
              ...
              src: options.src || `https://api.birlikbala.kz/uploads/articles/${options["data-path"]}`,
            },
          }),
    }
  },
  ...
  addNodeView() {
    return ({ node }) => {
      ...
      const src = node.attrs["src"] || `https://api.birlikbala.kz/uploads/articles/${path}`
...
const MenuBar = ({ editor, onAddMedia }) => {
  ...
  const addVideo = () => {
    ...
    input.onchange = async (e) => {
      ...
      if (file.size > 100000 * 1024 * 1024) {
        toast.error(`Файл ${file.name} превышает лимит в 10000 МБ`)
        return
      }
    ...
    onUpdate: ({ editor }) => {
      ...
      Object.entries(article.filePreviews).forEach(([previewUrl, fileName]) => {
        updatedContent = updatedContent.replaceAll(
          previewUrl,
          `https://api.birlikbala.kz/uploads/articles/${fileName}`
        )
      })
