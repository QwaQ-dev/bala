...
const CustomImage = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    const src = HTMLAttributes["src"] || `http://backend:8080/uploads/articles/${path}`
    return [
      "img",
      {
        ...HTMLAttributes,
        ...
        ({ commands }) =>
          commands.insertContent({
            ...
            attrs: {
              ...
              src: options.src || `http://backend:8080/uploads/articles/${options["data-path"]}`,
            },
          }),
    }
  },
  ...
  addNodeView() {
    return ({ node }) => {
      ...
      const src = node.attrs["src"] || `http://backend:8080/uploads/articles/${path}`
    ...
    onUpdate: ({ editor }) => {
      ...
      Object.entries(article.filePreviews).forEach(([previewUrl, fileName]) => {
        updatedContent = updatedContent.replaceAll(
          previewUrl,
          `http://backend:8080/uploads/articles/${fileName}`
        )
      })
